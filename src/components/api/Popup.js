...
class FlavorForm extends React.Component {
  constructor(props) {
    ...
    fetch("http://127.0.0.1:8000/api/problem_detail", {
      method: "POST",
      headers: {
  ...
  onFinish = (values) => {
    ...
    fetch("http://127.0.0.1:8000/api/problem", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      ...
      <Row style={{display: "block"}}>
        ...
        {this.state.isRender && (
          ...
          Click me!
            ...
            <Card title={"Bài toán: " + this.state.name} extra={(<Button type="primary" style={{color: "white", background: "blue"}} htmlType="submit">Giải</Button>)}>
              <Title level={5}>{"Đề bài: " + this.state.topic}</Title>
              <Title level={5}>Nhập các giá trị:</Title>
                {this.state.variable.map((element, i) => {
                  return (
                    <Row>
                    <Title level={5} style={{paddingRight: "6px"}}>{element.name + " ="}</Title>
                    <Form.Item
                      name={element.name}
